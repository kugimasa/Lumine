...
#include "FluorescenceBrdf.hlsli"

// 蛍光マテリアル
[shader("closesthit")]
void ClosestHit_Fluorescence(inout HitInfo payload, Attributes attrib)
{
    ...
    // アルベドの取得
    float3 albedo = l_matCB.baseColor.rgb;
    // アルベドテクスチャを蛍光用のマスクとして利用
    float3 fluorMask = l_diffuseTex.SampleLevel(gSampler, vtx.uv, 0).rgb * l_matCB.fluorWeight;
    ...
    if (totalLights > 0)
    {
        ...
        if (!TraceShadowRay(payload.seed, payload.hitPos, lightDir, lightDist))
        {
            ...
            // NEEによる寄与計算
            float3 lambertDiffuse = payload.attenuation * (brdf * G / max(totalLightPdf, EPS)) * lightInfo.intensity * misWeight;

            // RGBからXYZへの変換
            float3 xyzIntensity = RGB_to_XYZ(lightInfo.intensity * G / max(totalLightPdf, EPS));
            // XYZからRGBへ変換
            float3 fluorBrdf = XYZ_to_RGB(TEXTYELL_ReductionXYZ(xyzIntensity)) * INV_PI;
            float3 fluorEmission = payload.attenuation * fluorBrdf;

            // LambertDiffuseとFluorの合成
            payload.color +=  lerp(lambertDiffuse, fluorEmission, fluorMask);
        }
    }
    ...
    // TODO: 蛍光用のThroughputの計算が必要
    // // 蛍光成分
    // float3 xyzThroughput = RGB_to_XYZ(payload.attenuation);
    // float3 fluorBrdf = XYZ_to_RGB(TEXTYELL_ReductionXYZ(xyzThroughput)) * INV_PI;
    // // 寄与の計算
    // float cosTheta = CalcCos(norm, wo);
    // payload.attenuation *= lerp(brdf, fluorBrdf, fluorMask) * cosTheta / max(samplePdf, EPS);
